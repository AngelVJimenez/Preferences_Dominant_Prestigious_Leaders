...
# open the file
dat<-readRDS(file = "wvs_ready.rds")
head(dat)

# MODELS TO PREDICT DOMINANCE
library(brms)
library(loo)


fit0<-brm(
  formula=DOMINANCEREV~1+(1|COUNTRY_NAME),
...
fit1<-brm(
  formula=DOMINANCEREV~mo(CLASSREV)+mo(INCOME)+mo(POLTICALIDEOLOGY)+GENDER+AGE+
    (1|COUNTRY_NAME),
  family=cumulative("logit"), data=dat, chains = 4, iter = 5000, cores=4)
...
fit4<-brm(formula=DOMINANCEREV~mo(LACK_CONTROLREV)+
            (1|COUNTRY_NAME),
          family=cumulative("logit"), data=dat, chains = 4, iter = 5000, cores=4)
...
fit8<-brm(formula=DOMINANCEREV~mo(INTERCOUNTRYWARREV)+mo(CIVILWARREV)+mo(TERRORISTATTACKREV)+
            (1|COUNTRY_NAME),
          family=cumulative("logit"), data=dat, chains = 4, iter = 5000, cores=4)
...
fit9<-brm(formula=DOMINANCEREV~mo(CLASSREV)+mo(INCOME)+mo(POLTICALIDEOLOGY)+GENDER+AGE+ 
